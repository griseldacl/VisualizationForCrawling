 ...
 * 
 *  --- CVS Information ---
 *  $Id: ConnectionObserver.java,v 1.14 2005/11/13 10:06:32 gregork Exp $
 */
 ...
 * these connection still respond to incomming ping. In case they dont respond
...
public class ConnectionObserver implements Runnable
{
    ...
    public void run()
    {
        ...
        while( true )
        {
            ...
            if ( quiteList.size() > 0 )
            {
                ...
                {
                    Host host = (Host)quiteList.get( i );
                ...
                for ( int i = 0; i < size; i++ )
                {
                    ...
                    {
                        host.setStatus( HostConstants.STATUS_HOST_ERROR,
    ...
    private ConnectionSnapshoot findSnapshoot( Host host )
    {
        ...
        for ( int i = 0; i < size; i++ )
        {
            ConnectionSnapshoot shoot = (ConnectionSnapshoot)snapshootList.get( i );
            if ( shoot.host == host )
            {
                return shoot;
            }
