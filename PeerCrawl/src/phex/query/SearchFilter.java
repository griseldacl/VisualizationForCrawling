...
public class SearchFilter
{
    ...
    public XJBSearchFilter createXJBSearchFilter()
        throws JAXBException
    {
        // jaxb-beta way
        ObjectFactory objFactory = new ObjectFactory();
        XJBSearchFilter xjbFilter = objFactory.createXJBSearchFilter();
        xjbFilter.setName( name );
        xjbFilter.setRefineText( filterString );
        xjbFilter.setMinFileSize( minFileSize );
        xjbFilter.setLastTimeUsed( lastTimeUsed );
        xjbFilter.setMediaType( MediaType.convertToXJBMediaType( mediaType ) );
        return xjbFilter;
    }

    public static SearchFilter createFromxJBSearchFilter( XJBSearchFilter xjbFilter )
    {
        SearchFilter filter = new SearchFilter( xjbFilter.getName() );
        filter.filterString = xjbFilter.getRefineText();
        filter.minFileSize = xjbFilter.getMinFileSize();
        filter.lastTimeUsed = xjbFilter.getLastTimeUsed();
        filter.mediaType = MediaType.convertFromXJBMediaType(
            xjbFilter.getMediaType() );
        return filter;
    }
