 ...
 *  --- CVS Information ---
 *  $Id: FileSystemTableCellRenderer.java,v 1.5 2005/10/03 00:18:27 gregork Exp $
 */
...
public class FileSystemTableCellRenderer extends FWTableCellRenderer
{
    ...
    public FileSystemTableCellRenderer()
    {
        ...
        defaultIcon = factory.getIcon("File");
        defaultGrayIcon = HalfGrayedImageFilter.createGrayIcon(factory.getIcon("File"));
    }
    ...
    protected void setValue( Object value )
    {
        try
        {
            if ( value instanceof ShareFile )
            {
                ...
                if ( file.exists() )
                {
                    Icon icon = (Icon)shareFileIconMap.get( shareFile );
                    if ( icon == null )
                    {
            ...
            else if ( value instanceof File )
            {
                ...
                if ( file.exists() )
                {
                    ...
                    if ( grayIcon == null )
                    {
                        grayIcon = HalfGrayedImageFilter.createGrayIcon( icon );
        ...
        {
            NLogger.error(NLoggerNames.USER_INTERFACE, th);
        }
    }
