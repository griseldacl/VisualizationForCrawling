 ...
 * 
 *  --- CVS Information ---
 *  $Id: TestHostAddress.java,v 1.14 2005/11/20 23:04:25 gregork Exp $
 */
package phex.test;
...
public class TestHostAddress extends TestCase
{
    ...
    public void testIPClass()
    {
        try
        {
            ...
            for ( int i = 1; i < 128; i++ )
            {
                ...
                assertEquals( String.valueOf( i ), IpAddress.CLASS_A, 
                    address.getIpAddress().getIPClass() );
            }
            for ( int i = 128; i < 192; i++ )
            {
                address = new DefaultDestAddress(  i+ ".1.1.1",80 );
                assertEquals( String.valueOf( i ), IpAddress.CLASS_B,
                    address.getIpAddress().getIPClass() );
            }
            for ( int i = 192; i < 224; i++ )
            {
                address = new DefaultDestAddress(  i+ ".1.1.1",80 );
                assertEquals( String.valueOf( i ), IpAddress.CLASS_C, 
                    address.getIpAddress().getIPClass() );
            }
        }
        catch ( Exception exp )
    ...
    public void testIsIPValid() throws Exception
    {
        ...
        assertFalse( address.getIpAddress().isValidIP() );
        address = new DefaultDestAddress( "300.300.300.300",80 );
        assertFalse( address.getIpAddress().isValidIP() );
    }
    ...
    public void testHashEquals()
    {
        try
        {
            DefaultDestAddress hostaddress = new DefaultDestAddress( "127.0.0.1", 4444 );
            DefaultDestAddress hostaddress2 =  new DefaultDestAddress( "127.0.0.1", 4444 );
    ...
    {
        DefaultDestAddress address = new DefaultDestAddress("google.de", 80);
