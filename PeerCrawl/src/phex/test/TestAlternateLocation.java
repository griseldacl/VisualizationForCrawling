...
public class TestAlternateLocation extends TestCase
{
    ...
    public void testParse()
    {
        ...
        loc = AlternateLocation.parseUriResAltLoc( uriResStr2 );
        ...
        loc = AlternateLocation.parseUriResAltLoc( altLocStr );
    ...
    public void testAlternateLocationContainer()
        throws Exception
    {
        AlternateLocationContainer container = new AlternateLocationContainer(
            new URN( urnStr ) );
        ...
        assertEquals( 10,
            AlternateLocationContainer.parseCompactIpAltLocFromHTTPHeaders(
            new HTTPHeader[]{header}, new URN(urnStr) ).size() );
        ...
        assertEquals( 10, AlternateLocationContainer.parseCompactIpAltLocFromHTTPHeaders(
            new HTTPHeader[]{header}, new URN(urnStr) ).size() );
        assertEquals( 10, altLocSet.size() );
        ...
        container = new AlternateLocationContainer(
            new URN( urnStr ) );
        ...
        header = container.getAltLocHTTPHeaderForAddress( GnutellaHeaderNames.X_ALT,
            new DefaultDestAddress( "2.1.1.1", 6347 ), altLocSet );
        assertEquals( 1, AlternateLocationContainer.parseCompactIpAltLocFromHTTPHeaders(
            new HTTPHeader[]{header}, new URN(urnStr) ).size() );
        ...
        loc = AlternateLocation.parseUriResAltLoc(
            "http://1.1.1.2:6347/uri-res/N2R?urn:sha1:PLSTHIPQGSSZTS5FJUPAKUZWUGYQYPFB"
            + " 2002-04-30T08:30:10Z");
        assertNotNull( loc );
        container.addAlternateLocation( loc );

        loc = AlternateLocation.parseUriResAltLoc(
            "http://1.1.1.2:6347/uri-res/N2R?urn:sha1:PLSTHIPQGSSZTS5FJUPAKUZWUGYQYPFB"
            + " 2002-04-30T08:30:11Z");
        assertNotNull( loc );
        container.addAlternateLocation( loc );
