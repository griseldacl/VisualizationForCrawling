 ...
 * 
 *  --- CVS Information ---
 *  $Id: TestThrottleController.java,v 1.8 2005/10/03 00:18:29 gregork Exp $
 */
package phex.test;
...
public class TestThrottleController extends TestCase
{
    ...
    public void testThroughput2() throws Exception
    {
        BandwidthController controller =
            BandwidthController.acquireBandwidthController( "Test2", 1000 );

        long start = System.currentTimeMillis();
        long totalData = 10000;
        while( totalData > 0 )
        {
            int available = controller.getAvailableByteCount( true );
            int toUse = Math.min( available, 1000 );
            totalData -= toUse;
