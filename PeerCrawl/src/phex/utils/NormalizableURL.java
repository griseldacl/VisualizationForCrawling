 ...
 *  --- CVS Information ---
 *  $Id: NormalizableURL.java,v 1.3 2005/10/03 00:18:29 gregork Exp $
 */
package phex.utils;
 ...
 * @version Revision: 1.8 Date: 2004/06/20 00:08:15
 */
...
public final class NormalizableURL implements Serializable
{
     ...
     *  cannot be parsed successfully
     */
    public NormalizableURL(NormalizableURL context, String spec) throws MalformedURLException
    {
        ...
        try
        {
                    ...
                    .equalsIgnoreCase(context.getProtocol())))
            {
                ...
                if ((context.getPath() != null)
                    && (context.getPath().startsWith("/"))) newProtocol = null;
                if (newProtocol == null)
                {
    ...
    public boolean equals(Object obj)
    {
        ...
        if (!(obj instanceof NormalizableURL)) return (false);
        NormalizableURL other = (NormalizableURL) obj;
        if (!sameFile(other)) return (false);
    ...
    public boolean sameFile(NormalizableURL other)
    {
        ...
        if (!compare(protocol, other.getProtocol())) return (false);
        if (!compare(host, other.getHost())) return (false);
        if (port != other.getPort()) return (false);
        if (!compare(file, other.getFile())) return (false);
        return (true);
