 ...
 * 
 *  --- CVS Information ---
 *  $Id: GUIDRoutingTable.java,v 1.9 2005/10/03 00:18:29 gregork Exp $
 */
package phex.utils;
...
/**
 * This GUIDRoutingTable is used to route replies comming from the GNet
 * back to the requestor. This is done by using the GUID of the reply
 * which matches the GUID of the request to identify the correct 
 * route back to the requestor.
 */
public class GUIDRoutingTable
{
     ...
     * retrive the host from the idToHostMap. This extra layer is used to solve
     * three problems:<br>
     ...
     * - We are still able to identify dublicate query GUIDs even though the host
     ...
     * allows us to accomplish a very fast and efficient FIFO behaivor that stores
    ...
    public GUIDRoutingTable( long lifetime )
    {
        ...
        nextId = 0;
        currentMap = new TreeMap( new GUID.GUIDComparator() );
        lastMap = new TreeMap( new GUID.GUIDComparator() );
        idToHostMap = new HashMap();
        hostToIdMap = new HashMap();
    }
    ...
    public synchronized void addRouting( GUID guid, Host host )
    {
		...
		IntObj id = getIdForHost( host );
        
        // try to find and remove existing entry obj in currentMap and lastMap
		Entry entry = (Entry)currentMap.remove( guid );
		if ( entry == null )
		{
			entry = (Entry)lastMap.remove( guid );
	        if ( entry == null )
	        {
    ...
    {
		IntObj id = (IntObj) hostToIdMap.get( host );
        if ( id != null )
        {
    ...
    /**
     * Trys to find the reply route for the given GUID.
    ...
    {
		Entry entry = (Entry)currentMap.get( guid );
        if ( entry == null )
        {
			entry = (Entry)lastMap.get( guid );
        }
        if ( entry != null )
        {
            // returns null if there is no host for the id anymore.
            return (Host)idToHostMap.get( entry.hostId );
        }
        else
        {
            return null;
