 ...
 * 
 *  --- CVS Information ---
 *  $Id: GWebCacheConnection.java,v 1.20 2005/11/13 10:28:50 gregork Exp $
 */
...
public class GWebCacheConnection
{
    ...
    public boolean sendPingRequest()
    {
        try
        {
            ...
            {
                NLogger.warn( NLoggerNames.GWEBCACHE, "Host is null" );
                isCacheBad = true;
                return false;
            }
    ...
    public DestAddress[] sendHostFileRequest()
    {
        try
        {
            ...
            while ( line != null )
            {
                ...
                {
                    NLogger.debug( NLoggerNames.GWEBCACHE, "Invalid host found: " + line );
                    falseHostFound = true;
                    line = reader.readLine();
                    continue;
                }
            ...
            NLogger.debug( NLoggerNames.GWEBCACHE, "Hosts found: " + hostFileArr.length );
            return hostFileArr;
        }
        catch ( IOException exp )
        {
    ...
    public URL[] sendURLFileRequest()
    {
        try
        {
            ...
            NLogger.debug( NLoggerNames.GWEBCACHE, "URLs found: " + urlFileArr.length );
            return urlFileArr;
        }
        catch ( IOException exp )
        {
        ...
        throws IOException
    {
        ...
        if (   ServiceManager.sCfg.isHttpProxyUsed 
            && !StringUtils.isEmpty(ServiceManager.sCfg.httpProxyHost ) )
        {
            client.getHostConfiguration().setProxy(ServiceManager.sCfg.httpProxyHost, 
                ServiceManager.sCfg.httpProxyPort );
        }
        ...
        NLogger.debug(NLoggerNames.GWEBCACHE, "Open GWebCache connection to " +
            requestURL + "." );
        int responseCode = client.executeMethod(method);      
        NLogger.debug(NLoggerNames.GWEBCACHE, "GWebCache " +
    ...
    private void closeConnection()
    {
        ...
        NLogger.debug( NLoggerNames.GWEBCACHE, "Releasing HTTP method." );
    }
}