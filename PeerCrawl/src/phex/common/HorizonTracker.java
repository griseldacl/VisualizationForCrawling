 ...
 * 
 *  --- CVS Information ---
 *  $Id: HorizonTracker.java,v 1.8 2005/11/03 23:30:14 gregork Exp $
 */
package phex.common;
...
{   
     ...
     */
    private Set/*<HostAddress>*/ trackedAddresses;
    ...
    public synchronized void trackPong( PongMsg pong)
    {
        ...
        DestAddress address = new DefaultDestAddress( pong.getIP(), pong.getPort() );

        boolean isAdded = trackedAddresses.add( address );
        if ( isAdded )
        { 
            currentFileCount += pong.getFileCount();
            currentFileSize += pong.getFileSizeInKB();
            currentHostCount ++;
            Peer.getInstance().recomputeCrawlRange(getTotalHostCount()+1);
        }
    }
    ...
    private class TrackerRefreshTimer extends TimerTask
    {
         ...
         */
        private static final long TIMER_PERIOD = 1*60*1000;
